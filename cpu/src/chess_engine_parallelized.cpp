...
#include "../include/tt_parallel.h"
#include "../include/thread_local_data.h"
#include <array>
#include <algorithm>
#include <iostream>
#include <vector>
...
int negamax(Board &board, int depth, int alpha, int beta, int ply, 
    ...
    if (g_time_limit_ms > 0 && tld && (tld->nodes_searched & 4095) == 0) {
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::high_resolution_clock::now() - g_start_time).count();
        if (elapsed >= g_time_limit_ms) {
            g_stop_search.store(true, std::memory_order_relaxed);
            return 0;
        }
    }
    if (g_stop_search.load(std::memory_order_relaxed)) return 0;
...
Move find_best_move(Board& board, int max_depth, int time_limit_ms = 0) {
    ...
    g_stop_search.store(false, std::memory_order_relaxed);
...
// Global variable to store last search nodes count
